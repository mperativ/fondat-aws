...
def test_convert_dict_keys_to_snake_case():
    ...
    input_dict = {
        "outerCamel": {"innerCamel": 1, "innerPascal": 2},
        "outerPascal": {"innerCamel": 3, "innerPascal": 4},
    }
    expected = {
        "outer_camel": {"inner_camel": 1, "inner_pascal": 2},
        "outer_pascal": {"inner_camel": 3, "inner_pascal": 4},
    }
    assert convert_dict_keys_to_snake_case(input_dict) == expected

    # Test with lists
    input_dict = {"camelCase": [{"innerCamel": 1}, {"innerPascal": 2}]}
    expected = {"camel_case": [{"inner_camel": 1}, {"inner_pascal": 2}]}
    assert convert_dict_keys_to_snake_case(input_dict) == expected
